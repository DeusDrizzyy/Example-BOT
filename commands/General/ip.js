...
module.exports.run = (bot, message) => {
    fetch('https://api.mcsrvstat.us/2/' + serverIP).then(res => res.json()).then(body => {
        ...
        let emBed = new MessageEmbed()
            ...
            .setDescription(commands.ip.embedMessages.Description.replace('{players}', players).replace('{serverIP}', serverIP).replace('{status}', status));
